...
def test_local_segment_compactor_publishes_shard_manifest(tmp_path: Path) -> None:
    segments_root = tmp_path / "segments"
    manifests_root = tmp_path / "manifests"
    ...
    compactor = LocalSegmentCompactor(segments_root=segments_root, manifests_root=manifests_root)
    compactor.compact(collection_id="events", output_segment_id="seg-compacted", generation=2)

    manifest = ManifestStore(manifests_root).load(collection_id="events", shard_id="shard-0")
    assert manifest is not None
    assert manifest.active_segment_ids == ["seg-compacted"]
